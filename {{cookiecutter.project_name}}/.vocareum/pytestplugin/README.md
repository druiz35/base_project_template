...
addopts = -ra
markers =
    vocareum_test_name(name): Custom name for the test to show in Vocareum report.
    vocareum_weight(value): Score weight for this test.
    vocareum_success_msg(msg): Message shown on test success.
    vocareum_failure_msg(msg): Message shown on test failure.
```
...
@pytest.mark.vocareum_test_name("Creation of Lambda function for events") #Name of the test (exercise)
@pytest.mark.vocareum_weight(5) #Exercise grade definition
@pytest.mark.vocareum_success_msg("Test 1 passed: Created Lambda function for events.")
@pytest.mark.vocareum_failure_msg("Test 1 failed: Expected Lambda function for events to be created. Please try again.")
def test_lambda_created():
    assert True # Mocking a passing test. Specify the respective assertion for your test.

@pytest.mark.vocareum_test_name("Creation of DynamoDB table for events") #Name of the test (exercise)
@pytest.mark.vocareum_weight(5) #Exercise grade definition
@pytest.mark.vocareum_success_msg("Test 2 passed: Created DynamoDB table for events.")
@pytest.mark.vocareum_failure_msg("Test 2 failed: Expected DynamoDB table for events to be created. Please try again.")
